...
function filesPage(p) {
  ...
  return `<div class="breadcrumb"><button class="source-link" data-nav="board">Projects</button> / <button class="source-link" data-root="${esc(p.root)}">${esc(p.project||p.root)}</button> / History & files</div>
    ...
    <div class="revision-bar"><label for="file-revision">Version</label><select id="file-revision" data-file-revision ${FILES.busy?'disabled':''}><option value="">Working tree</option>${FILES.revision&&!(history?.revisions||[]).some(v=>v.revision===FILES.revision)?`<option value="${esc(FILES.revision)}" selected>Selected commit · ${esc(FILES.revision.slice(0,7))}</option>`:''}${(history?.revisions||[]).map(v=>`<option value="${v.revision}" ${v.revision===FILES.revision?'selected':''}>${esc(v.at)} · ${esc(v.label)} · ${v.revision.slice(0,7)}</option>`).join('')}</select><button class="source-link" data-file-reload>Reload file</button></div>
